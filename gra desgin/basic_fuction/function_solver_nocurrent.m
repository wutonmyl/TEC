function [T_chip,Tc,Th,To,Ti,Tg,h,flag,n,choice] = function_solver_nocurrent(par,T_g_in,i)
%FUNCTION_SOLVER 此处显示有关此函数的摘要
%   此处显示详细说明
rng default
x = optimvar('x',6,'LowerBound',[50,50,50,50,100,100],'UpperBound',[2000,2000,2000,2000,930,600]);
%第一个式子关于pchip还需要修改
eq1 = par.P_chip==par.k_ct*par.a_te*(x(1)-x(2));
eq2 = par.n*(par.k_p*par.a_copper*(x(2)-x(3))/par.delta_p)==...
      par.k_ct*par.a_te*(x(1)-x(2));
eq3 = par.n*(par.k_p*par.a_copper*(x(2)-x(3))/par.delta_p)==...
      par.k_ct*par.a_te*(x(3)-x(4));
eq4 = par.n*(par.k_p*par.a_copper*(x(2)-x(3))/par.delta_p)==...
...
eq5 = par.n*(par.k_p*par.a_copper*(x(2)-x(3))/par.delta_p)==...
      ls5;
