function [T_chip,Tc,Th,To,Ti,Tg,h,flag,n,choice] = function_solver_water(par,T_g_in,i)
%FUNCTION_SOLVER 此处显示有关此函数的摘要
%   此处显示详细说明
rng default
x = optimvar('x',6,'LowerBound',[50,50,50,50,273,273],'UpperBound',[800,800,800,800,930,600]);
%第一个式子关于pchip还需要修改
...
ls5 = fcn2optimexpr(@(x)Heat_transfer_coff_water(x(5),x(6),i,par)*par.a_g*(x(5)-x(6)),x);
eq5 = par.n*(par.alpha*par.I*x(3)+0.5*par.I^2*par.R+par.k_p*par.a_copper*(x(2)-x(3))/par.delta_p)==...
      ls5;
ls6 = fcn2optimexpr(@(x)par.mg*(refpropm('H','T',x(6),'P',par.P,'water')-refpropm('H','T',T_g_in,'P',par.P,'water')),x);
eq6 = ls6==ls5;
...
% Tg = sol.x(6);
% flag = exitflag;
[h,n,choice] = Heat_transfer_coff_water(Ti,Tg,i,par);
