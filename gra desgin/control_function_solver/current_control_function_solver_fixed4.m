...
rng default
% 定义优化变量(7个优化变量分布为，I,Tc,Th,To,Ti,Tg,Tchip)
mean_zero = mean(Tchip_record(:,j-1));
lb = [-10,100,100,185,185,185,mean_zero-50];
...
if i == 1
    ...
    Tchip_record_tem = Tchip_record(1:i-1,2);
    %前i项平均
    mean_i = (sum(Tchip_record_tem) + x(7))/i;
    %前i-1项减去平均的数组
    Tchip_record_tem_diff = Tchip_record_tem - mean_i;
    var_i = (sum(Tchip_record_tem_diff.*Tchip_record_tem_diff)+(x(7)-mean_i)^2)/i;
    var_i = fcn2optimexpr(@(x)var_i,x);
end

%能量为正作为第一个约束
limit1 = par.n*(par.alpha*x(1)*(x(3)-x(2))+par.R*x(1)^2) >= 0;
%方差小于对应零电流序列作为第二个约束
limit2 = var(Tchip_record(1:i,1))-var_i >= 0;
%温度距离零电流均温不要太大
limit3 = (x(7)-mean_zero)^2 - (Tchip_record(i,1)-mean_zero)^2 <= 0;
%增加梯度项
...
if i == 1
    target = a*(par.alpha*x(1)*(x(3)-x(2)));
else
    %把连续性要求也放到里面去
    target = a*(par.alpha*x(1)*(x(3)-x(2))+par.R*x(1)^2)^2+b*(I_record(i-1,j)-x(1))^2;
end
