function [I,Tc,Th,To,Ti,Tg,h,flag,n,choice] = current_control_function_solver_fixed(par,T_g_in,i,j,T_chip,mode,h,slope)
%   这里要把芯片温度变成电流
%   此处显示详细说明
rng default
% gs = GlobalSearch;
x = optimvar('x',6,'LowerBound',[0,100,100,100,100,100],'UpperBound',[10,800,800,800,930,600]);
%第一个式子关于pchip还需要修改
exp1 = heat_change(i,mode,h,slope)-par.k_ct*par.a_te*(T_chip-x(2));
...
Options = optimoptions("fmincon",'Display','iter','MaxFunctionEvaluations',3e4);
