function [data,wrong_num_record,i_final] = current_control_set7_gpu(par,ub,lb,grad_bond,search_num,mode,hight,slope)
%%  本函数是用于整合原main5和main6函数的所有求解外设置部分，将求解流程整流为一整个输入输出函数，目的是用于新main5整体优化求取最小梯度限制值
%%  本函数在set5的基础上做对应修改，以适配更加灵活多变的调控范围，满足电流尽量小的调控目的
...
%% 构造数据记录
% diary('myDiaryFile');
Q_tec_record = zeros(289,search_num,'gpuArray');
COP_record = zeros(289,search_num,'gpuArray');
i_record = zeros(289,search_num,'gpuArray');
Dz_record = zeros(289,search_num,'gpuArray');
Tchip_record = zeros(289,search_num,'gpuArray');    
Tc_record   = zeros(289,search_num,'gpuArray');
Th_record   = zeros(289,search_num,'gpuArray');
To_record   = zeros(289,search_num,'gpuArray');
Ti_record   = zeros(289,search_num,'gpuArray');
Tg_record   = zeros(289,search_num,'gpuArray');
h_record    = zeros(289,search_num,'gpuArray');
q1_r_record = zeros(289,search_num,'gpuArray');
q2_l_record = zeros(289,search_num,'gpuArray');
q2_r_record = zeros(289,search_num,'gpuArray');
q3_l_record = zeros(289,search_num,'gpuArray');
q3_r_record = zeros(289,search_num,'gpuArray');
q4_r_record = zeros(289,search_num,'gpuArray');
q5_r_record = zeros(289,search_num,'gpuArray');
flag_record = zeros(289,search_num);
n_record = zeros(289,search_num,'gpuArray');
choice_record = zeros(289,search_num,'gpuArray');
I_record = zeros(289,search_num,'gpuArray');
grad_record = zeros(search_num,'gpuArray');
range_record = zeros(search_num,'gpuArray');
Cp_g_record = zeros(search_num,'gpuArray');
density_g_record = zeros(search_num,'gpuArray');
H_g_record = zeros(search_num,'gpuArray');
lamda_g_record = zeros(search_num,'gpuArray');
Pr_g_record = zeros(search_num,'gpuArray');
mu_g_record = zeros(search_num,'gpuArray');
T_chip_target_ub_record = zeros(search_num,'gpuArray');
T_chip_target_lb_record = zeros(search_num,'gpuArray');
grad = zeros(289,search_num,'gpuArray');
need_control_record = zeros(289,search_num,'gpuArray');
wrong_num_record = zeros(289,search_num,'gpuArray');
...
while Dz<=par.length
    ...
    [need_control,Tchip_target_recmd_ub,Tchip_target_recmd_lb] = judge_need_control_broad(Tchip_record,i,j,lb,ub,grad_bond);
    ...
    T_chip_target_ub_record(i,j) = Tchip_target_recmd_ub;
    T_chip_target_lb_record(i,j) = Tchip_target_recmd_lb;
    %命令行显示进度
    disp(i);
    disp(j);
    disp(['推荐温度上界为:',num2str(Tchip_target_recmd_ub)]);
    disp(['推荐温度下界为:',num2str(Tchip_target_recmd_lb)]);
    ...
    while need_control ~= 0
        ...
        % disp(contol_repeat);
        [I,Tc,Th,To,Ti,Tg,h,flag,n,choice,T_chip] = current_control_function_solver_fixed7(par,T_g_in,i,j,Tchip_target_recmd_ub,Tchip_target_recmd_lb,I_record,mode,hight,slope);
        disp(i);
        disp(j);
        disp(['推荐温度上界为:',num2str(Tchip_target_recmd_ub)]);
        disp(['推荐温度下界为:',num2str(Tchip_target_recmd_lb)]);
...
data.T_chip_target_ub_record = T_chip_target_ub_record;
data.T_chip_target_lb_record = T_chip_target_lb_record;
